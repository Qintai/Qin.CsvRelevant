...
{
    internal class ZipOperationDefault : IZipOperation
    {
        ...
        /// 压缩文件
        /// </summary>
        /// <param name="fileToZip">要压缩的文件全名,包含文件路径</param>
        /// <param name="zipedFile">压缩后的文件名，没有路径</param>
        /// <param name="password">密码</param>
        ...
        /// <summary>
        /// Ensure that the project injects the "GB2312" font package with the code: "Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);"
        /// </summary>
        public bool ZipFileByGb2312(string fileToZip, string zipedFile, string password)
        {
            ...
            try
            {
                StreamReader streamReader = new StreamReader(fileToZip, System.Text.Encoding.GetEncoding("gb2312"));
                ...
                if (!string.IsNullOrEmpty(password))
                    zipStream.Password = password; // 设置密码了，net6 会报错 暂时不兼容
        ...
        [Obsolete("Stream or byte incoming is not supported")]
        private Stream ZipFile(byte[] inputbyte, string zipedFile, string password)
        {
            Stream resultstream;
            ZipOutputStream zipStream = null;
            ZipEntry ent = null;
            ...
            try
            {
                Stream stream = new MemoryStream(inputbyte);
                ...
                ent = new ZipEntry(Path.GetFileName(zipedFile)); // 这里不应该是 zipedFile，但是这里不能拿取 stream
                zipStream.PutNextEntry(ent);
                zipStream.SetLevel(6);
